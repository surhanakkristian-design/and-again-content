...
def main():
    ...
    for eid, langs in sent.items():
        ...
        for lang, (intro, ca, d1, d2) in langs.items():
            ...
            # The `(ca or d1)` guard makes this consistent with check_lang.py and validate_part.py:
            # a §0e article row in sk/cz/ua is empty in ALL THREE cells, and an empty distractor_2
            # there is the ruling, not a missing option. Without it the row is still refused and the
            # two guards above are removed for nothing. Corrected 15.9.2026 after the gate test
            # refused 36 of 36 article rows.
            if oc == 3 and not d2 and (ca or d1):
                errors.append(f"{tag}: options_count 3 but distractor_2 empty")
            if oc == 2 and d2:

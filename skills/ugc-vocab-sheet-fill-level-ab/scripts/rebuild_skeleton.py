...
Main-block exercises only (grammar + the level's main-block vocabulary types: A 27; B 68 and 69,
BRIEF §0r). The Simple Explanation (74/75) is
...
MAIN_VOCAB = {"A": (27,), "B": (68, 69)}   # §0r; 74/75 come later from add_simple_explanation.py
...
def main(a):
    ...
    other = sorted(m for m, k, p in media if X.is_other_pos(p, level))

    rows, eid = [], a.exercise_id_start
    for m in order:
        for t in sorted(assign[m]) + list(MAIN_VOCAB[level]):
    ...
    per_kind = Counter()
    for m in md:
        per_kind[s(md[m][5])] += len(assign[m]) + len(MAIN_VOCAB[level])
    ...
    print(f"  by media kind (grammar + {'/'.join(map(str, MAIN_VOCAB[level]))}): {dict(per_kind)}")
    ...
    print(f"  words outside noun/verb/adjective/adverb: {len(other)} {other[:10]}")
    if X.WEIGHTS[level]:
        tiers = {w: sum(counts.get(t, 0) for t in ts) for w, ts in X.TIERS_B.items()}
        per = {w: tiers[w] / len(X.TIERS_B[w]) for w in tiers}
        print(f"  tiers (weight: exercises / types = mean per type): "
              + "  ".join(f"w{w}: {tiers[w]}/{len(X.TIERS_B[w])} = {per[w]:.2f}" for w in (3, 2, 1)))
        print(f"  observed ratio per type, tier 3:2:1 = {per[3]/per[1]:.2f} : {per[2]/per[1]:.2f} : 1")

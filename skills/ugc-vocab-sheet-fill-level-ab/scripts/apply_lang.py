...
# validate_part.py and check_lang.py. The type sets come from there too.
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
import lang_scope

LANGS8 = ["sk", "de", "cz", "fr", "es", "ua", "tr", "hu"]
LANGS9 = LANGS8 + ["en"]
NO_INTRO = set(lang_scope.NO_INTRO_TYPES)
STEM = set(lang_scope.STEM_TYPES)
...
def main():
    ...
    for eid, langs in sent.items():
        ...
        try:
            ...
            errors.append(f"E {eid}: {exc}"); continue
        for lang, (intro, ca, d1, d2) in langs.items():
            tag = f"E {eid}/{lang}"
            if lang not in scope:
                errors.append(f"{tag}: §0p — type {t} is translated into {sorted(scope)} in this "
                              f"part; {lang} is out of scope and must stay empty")
                continue
        ...
        seen = {}
        for lang in sorted(l for l in langs if l in scope):
            k = langs[lang]
            if k in seen:

-- ============================================================================
-- 5b pass N/sk: append the final mark where the intro ended on the blank and the live
-- full_sentence has none (population: sk rows with full_sentence not null, intro_text
-- ending on '...', exercise_type <> 69, full_sentence not ending in . ? !, hold-out ids
-- excluded). Expected about 2231 rows. Mark = '.' where the English sibling ends in
...
where el.language_code = 'sk' and el.full_sentence is not null and e.exercise_type_id <> 69
...
from ok
where el.id = ok.id and el.language_code = 'sk'

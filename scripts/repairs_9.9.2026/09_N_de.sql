-- ============================================================================
-- 5b pass N/de: append the final mark where the intro ended on the blank and the live
-- full_sentence has none (population: de rows with full_sentence not null, intro_text
-- ending on '...', exercise_type <> 69, full_sentence not ending in . ? !, hold-out ids
-- excluded). Expected about 5 rows. Mark = '.' where the English sibling ends in
...
where el.language_code = 'de' and el.full_sentence is not null and e.exercise_type_id <> 69
  and rtrim(el.intro_text) like '%...' and el.full_sentence !~ '[.?!]$' and el.id not in (40017, 40188);
...
from ok
where el.id = ok.id and el.language_code = 'de'

#!/usr/bin/env python3
"""Phase 1V blind judge packets — 0 model calls.

    python3 phase1v/trackA_set/build_packets_1v.py [--allow-partial]

900 real items + 80 hidden duplicate controls (20 per level, the same item under a second
opaque id), shuffled ACROSS levels with seed 20260923 and split into N <= 5 equal parts
(4 if every part stays under ~19k estimated tokens, otherwise 5):
    ...
    set/judge/JUDGE_TASK_1V.md     the judge's only entry point (absolute paths)
    set/packet_key_1v.json         {qid: {item, duplicate_of, sid, level, part, position}}
...
def build(data_dir=None, judge_dir=None, allow_partial=False):
    ...
    for p in range(n_parts):
        ...
        sizes.append({'part': p + 1, 'items': len(chunk),
                      ...
                      'path': path})
    safe_dump(key, os.path.join(HERE, 'packet_key_1v.json'))

    brief = os.path.join(judge_dir, 'JUDGE_BRIEF_1V.md')
    T = ['# JUDGE_TASK_1V — your whole task (Phase 1V)', '',
    ...
    open(os.path.join(judge_dir, 'JUDGE_TASK_1V.md'), 'w',
    ...
    rep = {'items': len(items), 'duplicate_controls': len(dups),
           ...
           'levels_per_part': [dict(collections.Counter(
               ...
               for p in range(n_parts)],
           'key_path': os.path.join(HERE, 'packet_key_1v.json'),
           'judge_task': os.path.join(judge_dir, 'JUDGE_TASK_1V.md')}
    safe_dump(rep, os.path.join(HERE, 'build_packets_1v.json'))

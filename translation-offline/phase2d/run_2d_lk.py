...
A3  merge: out/lk_corrected_sk.jsonl + out/annotations_sk_final.jsonl (only lk[0] / lk_verdict / lk_reason
    replaced; every other field round-trip asserted byte-identical).
A4  gate: out/GATE3_2d.json + .md - 2C's gate-3 code path (AG v4, reader_nom, ERROR-of-decided, bar 12 %)
...
"""
import argparse, collections, csv, hashlib, json, math, os, random, re, subprocess, sys, threading, time
...
GATE3_SEED = 20260920
GATE3_N = 250
BATCH_IDS = ["sk_0001", "sk_0002", "sk_0003", "sk_0004", "sk_0005"]

LEDGER = os.path.join(H, "ledger_2d_lk.json")
LOGP = os.path.join(H, "run_2d_lk.log")
DEFECTS = os.path.join(H, "defects_2d_lk.txt")
SHAREF = os.path.join(H, "SHA_phase2c_out_before.txt")
...
def load_rows():
    ...
    assert len(rows) == 4064, "expected 4064 rows, got %d" % len(rows)
    return rows, raw
...
def session(sid, bid, rows):
    ...
    try:
        ...
        commit([sess_path(sid), LEDGER], "Phase 2D: lk session %s (%d tok)" % (sid, tok_total))
        if out is None:
...
def merge(rows, raw, tasks):
    ...
    cp_path = os.path.join(OUT, "lk_corrected_sk.jsonl")
    with open(cp_path, "w", encoding="utf-8") as f:
        for c in corrected: f.write(json.dumps(c, ensure_ascii=False) + "\n")

    # annotations_sk_final.jsonl - only lk[0] / lk_verdict / lk_reason replaced
    byn = {c["n"]: c for c in corrected}
    fin_path = os.path.join(OUT, "annotations_sk_final.jsonl")
    merged = []
    ...
    commit([cp_path, fin_path], "Phase 2D: lk corrections + final sk annotations (%d rows)" % len(merged))
...
def gate3(anns):
    ...
    g = {"phase": "2d", "sample": len(pick), "seed": GATE3_SEED, "rows_pool": len(anns), "bar_pct": GATE3_BAR,
         ...
         "ts": time.strftime("%Y-%m-%dT%H:%M:%S")}
    json.dump(g, open(os.path.join(OUT, "GATE3_2d.json"), "w", encoding="utf-8"), ensure_ascii=False, indent=1)
    ...
    open(os.path.join(OUT, "GATE3_2d.md"), "w", encoding="utf-8").write("\n".join(md))
    log("GATE3", json.dumps({k: g[k]["ERROR_of_decided_pct"] for k in ("agv4", "reader_nom")}))
    commit([os.path.join(OUT, "GATE3_2d.json"), os.path.join(OUT, "GATE3_2d.md")],
           "Phase 2D: GATE 3 on 250 merged rows (seed %d)" % GATE3_SEED)
    return g
...
def verify_sha():
    ...
    for l in open(SHAREF, encoding="utf-8"):
        ...
        ref[name.strip()] = h
    now = {}
    for fn in sorted(os.listdir(C2OUT)):
        p = os.path.join(C2OUT, fn)
        if not os.path.isfile(p): continue
        now[fn] = subprocess.run(["shasum", "-a", "256", p], capture_output=True, text=True).stdout.split()[0]
    bad = {k: (ref.get(k), now.get(k)) for k in set(ref) | set(now) if ref.get(k) != now.get(k)}
...
def report(rows, corrected, gate, stats):
    ...
    rep = {"phase": "2d_lk", "par": PAR, "n_per_session": NSESS, "sessions_expected": 41,
           ...
           "ts": time.strftime("%Y-%m-%dT%H:%M:%S")}
    rp = os.path.join(OUT, "REPORT_2d_lk.json")
    json.dump(rep, open(rp, "w", encoding="utf-8"), ensure_ascii=False, indent=1)
    commit([rp, LOGP, LEDGER, DEFECTS], "Phase 2D: lk re-judge report (PAR=2, %d tok)" % STATE["tok"])
...
def main():
    ...
    assert len(tasks) == 41, "expected 41 sessions, got %d" % len(tasks)
    load_ledger()
    if A.selftest:
        ...
        os.remove(os.path.join(OUT, "GATE3_2d.json")); os.remove(os.path.join(OUT, "GATE3_2d.md"))

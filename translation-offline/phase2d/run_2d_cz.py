#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Phase 2D TASK B: finish the CZECH half of the 2C selection (4,064 rows, cz_0001..cz_0005).
...
are COPIED (never moved) into phase2d/out/sessions/ at startup, asserted complete per session id before
...
0 Gemini calls.  0 DB access.  No deploy, no migration, no push.  Writes only inside phase2d/.
...
H = os.path.dirname(os.path.abspath(__file__))                 # .../translation-offline/phase2d
...
LEDGER = os.path.join(H, "ledger_2d_cz%s.json" % DRY)
GATES = os.path.join(H, "gates_2d_cz%s.json" % DRY)
LOGP = os.path.join(H, "run_2d_cz%s.log" % DRY)
DEFECTS = os.path.join(H, "DEFECTS_run_2d_cz%s.md" % DRY)
...
def stop(reason, detail):
    ...
    save_ledger()
    commit([fn, LEDGER, GATES, LOGP], "Phase 2D: STOP %s" % reason)
    sys.exit(3)
...
def soft_stop(reason, detail):
    ...
    commit([fn, LOGP], "Phase 2D: soft stop %s" % reason)
...
def adopt_2c_sessions():
    """phase2c is READ ONLY: copy (never move) every finished cz session file into phase2d/out/sessions/."""
    src = os.path.join(C2, "out", "sessions")
    got, skipped = [], []
    if not os.path.isdir(src):
        defect("phase2c/out/sessions missing; nothing to adopt"); return got
    for fn in sorted(os.listdir(src)):
        if not re.match(r"^cz_\d{4}_s\d{2}_(v|rw)\.json$", fn):
            continue
        dst = os.path.join(SESSDIR, fn)
        if not os.path.exists(dst):
            shutil.copy2(os.path.join(src, fn), dst)          # COPY, never move
        sid = fn[:-5]
        try:
            d = json.load(open(dst, encoding="utf-8"))
            meta = d.get("meta") or {}
            ok = isinstance(d.get("rows"), list) and meta.get("exit") == 0
        except Exception:
            ok = False; meta = {}
        if ok:
            got.append(sid)
            STATE["sessions"][sid] = {"kind": meta.get("kind"), "tokens": 0, "tokens_spent_in_2c": meta.get("tokens"),
                                      "wall_s": meta.get("wall_s"), "exit": 0, "rows": len(d["rows"]),
                                      "asked": meta.get("rows_asked"), "adopted": True, "status": "ADOPTED-2C",
                                      "source": os.path.relpath(os.path.join(src, fn), REPO),
                                      "ts": time.strftime("%Y-%m-%dT%H:%M:%S")}
        else:
            skipped.append(sid)
    log("ADOPT-2C", len(got), "finished Czech session files copied from phase2c (read only) into",
...
def record_giveup(sid, kind, why, tok, wall, rows_out=None):
    ...
    commit([LEDGER], "Phase 2D: GIVEUP %s (%s)" % (sid, why))
...
def session(sid, kind, prompt, rows):
    ...
    try:
        ...
        commit([sess_path(sid), LEDGER], "Phase 2D: session %s (%d tok)" % (sid, tok))
        if out is None:
...
def write_uploads(bid, anns):
    ...
    commit([xp, cp], "Phase 2D: upload format candidates from batch %s" % bid)
...
def run_batch(bid, lang, rows):
    ...
    if missing:
        ...
        save_ledger(); commit([LOGP, LEDGER], "Phase 2D: partial batch %s" % bid)
        return "partial"
    anns = [assemble(r, drv[r["n"]], MV.get(r["n"]), RW.get(r["n"])) for r in rows]
    with open(ann_path, "w", encoding="utf-8") as f:
        for a in anns:
            f.write(json.dumps(a, ensure_ascii=False) + "\n")
    # change 6: sidecar header - Czech lk is corrected by the dedicated pass, not trusted from the v session
    side = os.path.join(OUT, "annotations_%s_%s.meta.json" % (lang, bid.split("_")[1]))
    json.dump({"batch": bid, "lang": lang, "rows": len(anns), "phase": "2D", "task": "B",
               "lk_needs_dedicated_pass": LK_NEEDS_DEDICATED_PASS,
               "lk_note": "Task A: the v-session lk_verdict is not trustworthy as a single sample. The v prompt is "
                          "unchanged (2C VPROMPT('cz') verbatim); Czech lk must be corrected by the same dedicated lk "
                          "pass Task A runs for Slovak.",
               "par": A["par"], "prompt_sha": PROMPT_SHA, "sessions": [t[0] for t in ALLTASKS],
               "adopted_from_2c": sorted(sid for sid in (t[0] for t in ALLTASKS)
                                         if (STATE["sessions"].get(sid) or {}).get("adopted")),
               "written": time.strftime("%Y-%m-%dT%H:%M:%S")},
              open(side, "w", encoding="utf-8"), ensure_ascii=False, indent=1)
    commit([ann_path, side], "Phase 2D: annotations %s (%d rows, lk_needs_dedicated_pass)" % (bid, len(anns)))
    ...
    save_ledger()
    commit([GATES, done_marker, LOGP, LEDGER, DEFECTS], "Phase 2D: batch %s complete + GATE 3" % bid)
...
def selftest():
    ...
    ck("backoff:at_most_3_retries", MAX_RETRY == 3)
...
def main():
    ...
    for bid, lang, rows in SELECTED:
        ...
        if st == "partial" and STATE["stop_spawn"]:
            break
    STATE["finished"] = True
    ...
    save_ledger()
    commit([LOGP, LEDGER, GATES, DEFECTS], "Phase 2D: run segment finished (%d tokens cumulative)" % STATE["tok"])

#!/usr/bin/env python3
"""Phase 1V assembler — adapted from phase1t/set/assemble_1t.py.  0 model calls.

    python3 phase1v/trackA_set/assemble_1v.py                 # validate + select + write
    python3 phase1v/trackA_set/assemble_1v.py --dry-run       # validate + select, write nothing

INPUT   phase1v/trackA_set/writers/writer_<A1|A2|B1|B2>_part{1,2}.json
...
IDS     public 1V001-1V100, sid 200001-200100, level blocks per SPLIT_1V.md
        (A1 = 1V001-025, A2 = 026-050, B1 = 051-075, B2 = 076-100); inside each block the
...
OUTPUT  <set>/data/{sentences,annotations,items}.json   (also mirrored to phase1v/data/)
        <set>/assemble_1v.json + ASSEMBLE_REPORT_1V.md
"""
import argparse, collections, glob, json, os, re, shutil, sys, unicodedata

sys.dont_write_bytecode = True
HERE = os.path.dirname(os.path.abspath(__file__))              # phase1v/set
PHASE = os.path.dirname(HERE)                                  # phase1v
...
def earlier_slovak():
    ...
    for p in paths:
        ...
        if rel.startswith('phase1v' + os.sep):          # our own phase: never a reference
            continue
        try:
...
def assign_ids(kept):
    """Level blocks per SPLIT_1V.md; inside a block parity alternates over the kept order."""
    out = []
...
def build(dry_run=False):
    ...
    floors_by_construction = {
        ...
        'F6_determiner': tag_counts['determiner']}
    ...
    safe_dump(rep, os.path.join(HERE, 'assemble_1v.json'))
    write_report(rep)
...
def write_report(rep):
    p = os.path.join(HERE, 'ASSEMBLE_REPORT_1V.md')
    L = ['# ASSEMBLE_REPORT_1V (0 model calls)', '',

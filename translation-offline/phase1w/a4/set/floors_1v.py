#!/usr/bin/env python3
"""Phase 1V floor check on JUDGED counts (SPLIT_1V.md) — 0 model calls.

    python3 phase1v/trackA_set/floors_1v.py

Reads data/items.json (writer tags) + data/labels.json (judge labels) and writes
set/floors_1v.json — copied byte-identically to set/FLOOR_CHECK_1V.json, the name
SPLIT_1V.md uses — with EXACTLY these keys:
...
Exit 0 when all eight floors hold (F6 added in 1V), 2 otherwise.  The run does not start on exit 2.
"""
...
def check(data_dir=None, out_dir=None):
    ...
    cnt = {
        ...
        'F5_missing_article_wrong': n(lambda i: tag(i, 'missing-article')
                                      and lab(i) == 'wrong'),
        'F6_determiner_correct': n(lambda i: tag(i, 'determiner')
                                   and lab(i) == 'correct')}
    ...
    for k in ('F1_agent_drops_wrong', 'F1a_fronted_wrong', 'F1b_misaligned_wrong',
              ...
              'F5_missing_article_wrong', 'F6_determiner_correct'):
        ok = cnt[k] >= MINS[k]
    ...
    out['missing_article_judged_correct'] = n(lambda i: tag(i, 'missing-article')
                                              and lab(i) == 'correct')
    out['determiner_judged_wrong'] = n(lambda i: tag(i, 'determiner') and lab(i) == 'wrong')
    ...
    out['drop_cells'] = {t: {'wrong': n(lambda i, t=t: tag(i, t) and lab(i) == 'wrong'),
                         ...
                         for t in DROP_TAGS}
    p = os.path.join(out_dir, 'floors_1v.json')
    safe_dump(out, p)
    shutil.copyfile(p, os.path.join(out_dir, 'FLOOR_CHECK_1V.json'))

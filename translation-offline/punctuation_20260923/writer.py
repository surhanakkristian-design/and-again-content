"""Guarded batched writes to exercise_localizations for the sentence-final punctuation pass (23.9.2026, decision 54).
...
Batches <= 500 rows, each ONE UPDATE statement = one transaction. Rollback SQL is written, never run.
"""
...
def run(name, changes, cols, dry=False, B=500):
    ...
    with open(rpath, "w") as f:
        f.write(f"-- rollback for punctuation_20260923/{name}; NOT run. Restores the backed-up values where the row still holds\n"

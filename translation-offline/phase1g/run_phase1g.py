#!/usr/bin/env python3
"""Phase 1g — three changes on top of the frozen Phase 1f stack, measured on the frozen Phase 1c sets
with gemini-3.1-flash-lite only.
  ...
  python3 phase1g/run_phase1g.py --selftest   # offline, no key, no network
  python3 phase1g/run_phase1g.py --plan       # needed calls vs the 1200 cap (offline)
  python3 phase1g/run_phase1g.py --calls      # model calls for anything the ledgers do not already have
  python3 phase1g/run_phase1g.py --tabulate   # offline ablation, re-runnable
  python3 phase1g/run_phase1g.py --all        # calls then tabulate

phase1c/, phase1e/, phase1f/ are READ-ONLY here. No previous phase's runner is imported: lib_prev.py is a
verbatim COPY of phase1e/run_phase1e.py whose HERE resolves to phase1g/, so every path constant inside it
(LEDGER, LOG, MODEL_CFG) points into phase1g/. Previous-phase ledgers are read with plain open().
...
assert os.path.dirname(os.path.abspath(base.__file__)) == HERE, 'lib_prev must live in phase1g/'
...
LEDGER = os.path.join(HERE, 'calls.jsonl')                # 1g's own ledger (new calls only)
...
base.LEDGER, base.LOG = LEDGER, CALL_LOG                  # every write of the copied module -> phase1g/
...
def f2_boundary_violation(it):
    ...
    span_in_answer = sorted(at & EN_SPAN) + digits
...
class Bud1g(base.Budget):
    def __init__(self):
        base.Budget.__init__(self)                       # reads phase1g/calls.jsonl only
...
def stage_calls():
    ...
    print('needed %d items x 2 variants; new calls required: %d (cap %d, spent in 1g so far %d)'
...
def stage_tabulate():
    ...
    counts = {'new': new_n, 'reused': used - new_n, 'failed': sum(1 for x in mine if x.get('counted')
              ...
              'cap': 1200, 'http_attempts_1g': len(mine),
              ...
              'note': 'reused = verdicts taken from the phase1e / phase1f ledgers with a byte-identical '
                      'prompt; new = HTTP calls made by Phase 1g'}
    ...
    md = ['# Phase 1g — F5 adjunct deletion, F4v2 Slovak-only subject guard, F2 tip boundary (%s)\n' % MODEL,

"""Guarded batched writes to exercise_localizations for the mismatch fix (23.9.2026).
...
def run(name, changes, cols, dry=False, B=200):
    ...
    with open(rpath, "w") as f:
        f.write(f"-- rollback for mismatch_fix/{name}; NOT run. Restores the backed-up values where the row still holds\n"

#!/usr/bin/env python3
"""Phase 1U label join — 0 model calls.

    python3 phase1u/set/join_labels_1u.py

Reads set/judge/verdicts_part*.json + set/packet_key_1u.json + data/items.json,
refuses loudly unless ALL key entries (900 real + 80 duplicate controls) are present and
well formed, and writes data/labels.json (mirrored to phase1u/data/) in the schema the
...
def join(data_dir=None, judge_dir=None, key_path=None, out_dirs=None, force=False):
    ...
    key_path = key_path or os.path.join(HERE, 'packet_key_1u.json')
    out_dirs = out_dirs or [data_dir, os.path.join(PHASE, 'data')]
    ...
    if fatal and not force:
        rep['REFUSED'] = True
        safe_dump(rep, os.path.join(HERE, 'join_labels_1u.json'))

# Phase 1U — fresh Slovak set, WRITER SPEC (you are a blind writer)
...
## OWNER'S RULING ON ARTICLES (Phase 1U)
...
- `c2` another faithful translation differing by a determiner choice (the / that / my where English
  allows it) or by a free within-frame tense choice. Tag `determiner` or `aspect`.
...
- `phase1u/set/writers/writer_<LEVEL>_part1.json` — s01 … s15
- `phase1u/set/writers/writer_<LEVEL>_part2.json` — s16 … s29
...
```json
{
  ...
  "answers": [
    ...
    {"aid": "c2", "kind": "C", "intent": "different determiner choice", "type": null,
     "tags": ["determiner"],
     "answer": "When the mayor opens that new bridge, the residents will hold a big celebration."},
...
sentences have a `missing-article` w4, and anything you could not do.

# Judge brief — Phase 1U (you are a blind judge: read ONLY this brief and the packet files named in your task message; open nothing else)
...
## Packets
The assembler writes `phase1u/set/judge/packet_partK.jsonl` (K = 1..N, N ≤ 5) — one JSON object per
line, each with the opaque item id, the Slovak sentence, the learner's English answer and the CEFR
level, and nothing else. `phase1u/set/judge/JUDGE_TASK_1U.md` lists the parts and the verdict file

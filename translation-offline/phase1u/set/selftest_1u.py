#!/usr/bin/env python3
"""Phase 1U set-tooling self-test — 0 model calls, synthetic data only.

    python3 phase1u/set/selftest_1u.py

Exercises join_labels_1u.join and floors_1u.check on a synthetic 900-item set in a scratch
...
import join_labels_1u as J                                     # noqa: E402
import floors_1u as F                                          # noqa: E402
...
def synth(tmp):
    ...
    for iid in dups:
        ...
        key['Q%04d' % q] = {'item': iid, 'duplicate_of': orig, 'sid': int(iid.split(':')[1]),
                            'level': 'A1', 'part': 1 + q % 4, 'position': q}
    kp = os.path.join(tmp, 'packet_key_1u.json')
...
def main():
    tmp = tempfile.mkdtemp(prefix='selftest_1u_')
    try:
        ...
        rep = json.load(open(os.path.join(HERE, 'join_labels_1u.json')))
        ...
        rc = F.check(data, tmp)
        fl = json.load(open(os.path.join(tmp, 'floors_1u.json')))
        ...
        ok('FLOOR_CHECK_1U.json written too',
           os.path.exists(os.path.join(tmp, 'FLOOR_CHECK_1U.json')))

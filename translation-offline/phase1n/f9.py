#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PHASE 1L COPY of phase1k/taskB/f9.py — ONE fix (2.2): the negated-PRESENT
l-participle misread (sid 140006 "neposiela"). Nothing else in F9 is changed.
...
IMPF_SUF = ('iela', 'ieľa', 'áva', 'ávam', 'ávaš', 'ávajú', 'ieva', 'ievam', 'ievajú', 'úva', 'úvam', 'úvajú',
            'ína', 'ínam', 'ínajú')
...
PRES_LA_SUF = ('iela', 'ieľa', 'iel', 'ieľ')
PRES_LA_LEX = set("""posiela vysiela zasiela odosiela rozosiela prisiela dosiela nasiela
strieľa usmieva""".split())


def _pres_la(w):
    """2.2 FIX: a finite PRESENT verb that merely ENDS like an l-participle.

    "(ne)posiela" is the 3sg present of "posielať"; the old ending test read the final "-la"
    as a feminine l-participle and dated the sentence to the past (sid 140006, the same
    ending-detection family as 20435 / 23669 / 24741). Slovak has no l-participle in
    -iela / -ieľa, so excluding that shape cannot hide a real past.
    """
    b = w[2:] if w.startswith('ne') and len(w) > 5 else w
    return b in PRES_LA_LEX or any(b.endswith(s) for s in PRES_LA_SUF)
...
def _is_l_part(toks, i):
    ...
    if w in L_NONVERB or w in DET:
        return False
    if _pres_la(w):
        return False                                  # 2.2 FIX: present "(ne)posiela"

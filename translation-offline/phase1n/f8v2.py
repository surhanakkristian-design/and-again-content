...
INITIAL_SKIP = {'by', 'som', 'si', 'sme', 'ste', 'sa', 'nie', 'ne', 'bol', 'bola', 'bolo',
                'boli', 'nebol', 'nebola', 'nebolo', 'neboli', 'bude', 'budú', 'budem', 'budeme',
                'budeš', 'budete', 'nebude', 'nebudú'}
...
def _verbs(t):
    ...
    for i, w in enumerate(t):
        if i and t[i - 1] in f9.PREPS:
            continue
        if w in STOP:
            continue                                   # adverb/particle, never a finite verb
...
def _caps(text):
    """lower-cased proper names of the clause: capitalised words that are NOT clause-initial.

    Slovak word order is free and case syncretism is wide, so a bare clause-initial noun is just
    as often the OBJECT ("ten balík odoslal" = "he sent the parcel").  Only a mid-clause capital
    is reliable enough to be asserted as the agent; everything else abstains.
    """
    ws = re.findall(r'[^\W\d_]+', text or '', re.UNICODE)
    return {w.lower() for w in ws[1:] if w[:1].isupper()}


def _noun_agent(t, vidx, caps):
    """The proper name before the first finite verb, scanning FORWARD from the clause start.

    Conservative: a preposition, an oblique ending or any non-closed-class token that is not a
    proper name makes the clause abstain.  (The old backwards scan from the verb returned
    adverbs - "dnes", "ráno", "asi" - and fronted objects as agents.)
    """
    if 'sa' in t:
        return None                      # reflexive/impersonal: the surface noun may be the patient
    for j in range(0, max(0, min(vidx, len(t)))):
        x = t[j]
        if x in f9.PREPS:
            return None                  # prepositional phrase before the verb -> abstain
        if x in STOP or x in f9.DET or x in PRON or x in INDEF:
            continue
        if len(x) <= 2 or any(x.endswith(e) for e in SK_OBL_END):
            return None
        if f9._looks_present(x) or x.endswith(('l', 'la', 'lo', 'li')):
            return None                  # verb-shaped token -> abstain
        return x if x in caps else None   # only a mid-clause proper name is asserted
    return None


def _clause_agent(t, text=''):
    ...
    # --- infinitive "byť" (modal or copular: "musí byť skutočný") and copular/perception verbs
    if 'byť' in t or 'byt' in t:
        return None, 'infinitive "byť" (modal/copular predicate) -> abstain'
    if any(w in STATE_V for w in t):
        return None, 'copular/perception verb "%s" -> abstain' % next(w for w in t if w in STATE_V)
    verbs = _verbs(t)
    if not verbs:
        return None, 'no finite verb signal -> abstain'
    # --- copular byť-only clause ("On je unavený.", "Terminál bol prázdny.")
    if verbs[0][1] in BYT_FIN and not any(k == 'lpart' and w not in BYT_FIN for _i, w, k in verbs):
    ...
    noun = _noun_agent(t, vidx, caps)
    if noun:
        return ({'tok': noun, 'kind': 'noun', 'en': None, 'person': 3, 'number': None, 'gender': None},
                'proper name "%s" before the active verb "%s"' % (noun, vtok))
    ...
    # --- pro-drop needs a clause-INITIAL finite verb: any other content token before the verb
    #     may itself be the (fronted) subject, which the morphology cannot contradict.
    if any(x not in STOP and x not in f9.DET and x not in PRON and x not in INITIAL_SKIP
           for x in t[:verbs[-1][0]]):
        return None, 'content token before the verb may be the subject -> abstain'
    ...
    if p == 3 and n == 'sg' and g == 'n':
        # subjectless neuter 3sg is the Slovak IMPERSONAL ("pršalo", "začalo pršať"): the English
        # "it" is an expletive, not an agent
        return None, 'subjectless neuter 3sg (impersonal/weather) -> abstain'
    en = _en_of(p, n, g)
    if not en:
        # 3sg present: the verb carries no gender, so no English subject can be asserted
        return None, 'pro-drop subject not recoverable from the verb ("%s") -> abstain' % vtok
...
def sk_clauses(slovak, annotation=None):
    ...
    for i, c in enumerate(f9._clauses(slovak)):
        ag, why = _clause_agent(c['toks'], c.get('text', ''))

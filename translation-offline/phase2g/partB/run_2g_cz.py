#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""PHASE 2G PART B: patched copy of phase2f/run_2f_cz.py; every change is in partB/CHANGES_2G.md.
...
BASE = os.path.dirname(os.path.dirname(H))    # 2G: partB is one level deeper
...
F2 = os.path.join(BASE, "phase2f")            # INPUT, read only (2G adopts from here first)
...
F2_ROWS = {json.loads(_l)["n"] for _l in open(os.path.join(F2, "out", "annotations_cz_final.jsonl"), encoding="utf-8") if _l.strip()}
...
def parse_argv(argv):
    a = {"all": False, "batches": [], "only": [], "dry": False, "cap": CAP, "par": PAR, "max_sessions": None,
         "ignore_stop": False, "self_test": False, "no_spawn": False}
    i = 0
    while i < len(argv):
        x = argv[i]
        if x == "--all": a["all"] = True
...
LEDGER = os.path.join(H, "ledger_2g_cz%s.json" % DRY)
GATES = os.path.join(H, "gates_2g_cz%s.json" % DRY)
LOGP = os.path.join(H, "run_2g_cz%s.log" % DRY)
DEFECTS = os.path.join(H, "DEFECTS_run_2g_cz%s.md" % DRY)
...
PROGRESS = os.path.join(H, "progress%s.log" % DRY)
...
def stop(reason, detail):
    ...
    save_ledger()
    pieces_report()
    commit([fn, LEDGER, GATES, LOGP, TIMELINE, PIECES_JSON, REFUSED_MD], "Phase 2G Part B: STOP %s" % reason)
    sys.exit(3)
...
def soft_stop(reason, detail):
    ...
    commit([fn, LOGP], "Phase 2G Part B: soft stop %s" % reason)
...
def save_ledger():
    ...
    d = {"cap": A["cap"], "spent": STATE["tok"], "par": A["par"], "concurrency": A["par"],
         ...
         "throughput_guard": "REMOVED in Phase 2G Part B (B1)",
...
# ---- 2G B1: ThroughputGuard / GUARD / monitor_loop REMOVED ENTIRELY (CHANGES_2G.md)
...
def adopt_prior_sessions():
    ...
    srcs = [("phase2f", os.path.join(F2, "out", "sessions")), ("phase2e", os.path.join(E2, "out", "sessions")), ("phase2d", os.path.join(D2, "out", "sessions")),
    ...
    for tag, src in srcs:
        ...
        for fn in sorted(os.listdir(src)):
            if not re.match(r"^cz_\d{4}_s\d{2}_(v|rw)(_h[12])?\.json$", fn) or fn in seen:
                continue
    ...
    commit([LEDGER] + [sess_path(x) for x in got],
           "Phase 2G Part B: adopt %d finished prior Czech sessions (copied; phase2c/phase2e untouched)" % len(got))
    return got
...
def record_giveup(sid, kind, why, tok, wall, rows_out=None):
    with LOCK:
        ...
        STATE["sessions"][sid] = {"kind": kind, "tokens": tok, "wall_s": round(wall, 1), "exit": None,
                                  ...
                                  "ts": time.strftime("%Y-%m-%dT%H:%M:%S")}
        save_ledger()
    tl("giveup", sid, kind=kind, why=why, tokens=tok)
    log("GIVEUP", sid, kind, why, "tok", tok, "wall", round(wall, 1), "- moving on, the run never hangs")
    commit([LEDGER], "Phase 2G Part B: GIVEUP %s (%s)" % (sid, why))

# ---------------------------------------------------------------- 2G Part B: timeline, hard Claude stops, pieces
_LAUNCH = [0]
INFLIGHT_SET = set()
...
def session(sid, kind, prompt, rows):
    ...
    with LOCK:
        ...
        if proj > A["cap"]:
            tl("cap_refused", sid, kind=kind, spent=STATE["tok"], reserved_inflight=STATE["inflight"] * max(EST.values()), est=e, cap=A["cap"])
    ...
    try:
        ...
        retry_tok = 0
        p = None
        _mr = PIECE_MAX_RETRY if _is_piece(sid) else MAX_RETRY
        for attempt in range(_mr + 1):
            ...
            post_call(sid, kind, attempt, p, _lidx)
            if should_retry(p.returncode, p.stdout, p.stderr):
                ...
                if attempt < _mr:
                    ...
                    log("RETRY", sid, kind, "attempt", attempt + 1, "of", _mr, "reason", why,
                        "sleep %.1fs" % d, "counted:false")
                    _sl0 = time.time()
                    time.sleep(d)
                    record_sleep(_sl0, time.time())        # 2F change 1: excluded from the guard's clock
                    continue
                record_giveup(sid, kind, "retries_exhausted after %d retries (%s)" % (_mr, why),
        ...
        with LOCK:
            ...
            if not over_tok:
                STATE["kind"][_sk][0] += 1; STATE["kind"][_sk][1] += tok; STATE["kind"][_sk][2] += wall
        ...
        commit([sess_path(sid), LEDGER], "Phase 2G Part B: session %s (%d tok)" % (sid, tok))
        if out is None:
...
def write_uploads(bid, anns):
    ...
    commit([xp, cp], "Phase 2G Part B: upload format candidates from batch %s" % bid)
...
def run_batch(bid, lang, rows):
    ...
    for sid, kind, prompt, crows in ALLTASKS:
        ...
        if d is None:
            ...
            half = {}
            for suf, sub in [("_h1", ns[:len(ns) // 2]), ("_h2", ns[len(ns) // 2:])] + [("_p%02d" % (k // PIECE_ROWS + 1), ns[k:k + PIECE_ROWS]) for k in range(0, len(ns), PIECE_ROWS)]:
                if not sub:
                    continue
    ...
    if not write_rows:
        ...
        save_ledger(); commit([LOGP, LEDGER, DEFECTS], "Phase 2G Part B: batch %s has no complete session" % bid)
        return "partial"
    ...
    commit([ann_path, side, DEFECTS], "Phase 2G Part B: annotations %s (%d of %d rows%s)"
    ...
    if first and False:                                   # 2G: B6 builds the only upload package
    ...
    commit([GATES, done_marker if not missing else os.path.join(OUT, "PARTIAL_%s.json" % bid), LOGP, LEDGER, DEFECTS],
           "Phase 2G Part B: batch %s %s + GATE 3" % (bid, "complete" if not missing else "partial"))
    for k in ("agv4", "reader_nom"):
        p = g[k]["ERROR_of_decided_pct"]
        if p is not None and p > GATE3_BAR and (A["dry"] or A["no_spawn"]):
...
def main():
    ...
    for bid, lang, rows in SELECTED:
        ...
        if st in ("done", "partial"):
            ...
            p = projection_2g(_vr, _rwn, time.time() - STATE["t0"]) if STATE["rows_paid"] > 0 else None
            if p is not None:
    ...
    save_ledger()
    commit([LOGP, LEDGER, GATES, DEFECTS], "Phase 2G Part B: run segment finished (%d tokens cumulative)" % STATE["tok"])

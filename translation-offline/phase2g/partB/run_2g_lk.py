...
def _samp(rnd, pop, k): return _samp(rnd, pop, min(k, len(pop)))   # 2G: fewer than 250 rows is legal
sys.dont_write_bytecode = True

H = os.path.dirname(os.path.abspath(__file__))
BASE = os.path.dirname(os.path.dirname(H))     # 2G: translation-offline
REPO = os.path.dirname(BASE)                   # and-again-content
C2 = os.path.join(BASE, "phase2c")
D2 = os.path.join(BASE, "phase2d")             # INPUT, read only
OUT = os.path.join(H, "lk")                    # 2G: only rows lacking a dedicated lk judgement (prep_lk.py)
...
LEDGER = os.path.join(H, "ledger_2g_lk.json")
LOGP = os.path.join(H, "run_2g_lk.log")
DEFECTS = os.path.join(H, "defects_2g_lk.txt")
SHAREF = os.path.join(os.path.dirname(H), "SHA_inputs_before.txt")   # 2G: phase2g/SHA_inputs_before.txt
...
# ---------------------------------------------------------------- 2G Part B: timeline + usage-limit hard stop
...
def session(sid, bid, rows):
    ...
    try:
        ...
        commit([sess_path(sid), LEDGER], "Phase 2G Part B: lk session %s (%d tok)" % (sid, tok_total))
        if out is None:
...
def merge(rows, raw, tasks):
    ...
    commit([cp_path, fin_path], "Phase 2G Part B: lk corrections + final sk annotations (%d rows)" % len(merged))
...
def gate3(anns):
    ...
    pick = _samp(rnd, anns, min(GATE3_N, len(anns)))
    ...
    commit([os.path.join(OUT, "GATE3_2e.json"), os.path.join(OUT, "GATE3_2e.md")],
           "Phase 2G Part B: GATE 3 on 250 merged rows (seed %d)" % GATE3_SEED)
    return g
...
def report(rows, corrected, gate, stats):
    ...
    commit([rp, LOGP, LEDGER, DEFECTS], "Phase 2G Part B: lk re-judge report (PAR=2, %d tok)" % STATE["tok"])
...
def main():
    ...
    if A.selftest:
        ...
        rnd = random.Random(GATE3_SEED); pick = _samp(rnd, anns, 250)
    ...
    if not A.merge_only:
        ...
        if flagged:
            ...
            commit([LOWDIR], "Phase 2G Part B: low-mode re-run of %d lk session(s)" % len(flagged))
        save_ledger()

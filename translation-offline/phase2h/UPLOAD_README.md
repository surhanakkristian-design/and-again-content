# Phase 2H upload package - Czech (`out/upload_cz_final.xlsx`)

Sheet **`cz`**, **4064 rows**, columns `exercise_id`, `language_code`, `level`, `src`, `en`, `structure_json`; the whole annotation as JSON in `structure_json`. Nothing was uploaded, nothing was written to the database.

* Rows in `out/annotations_cz_final.jsonl`: 4064; sources: {"phase2g_final": 2700, "lk_2h_dedicated": 1364}.
* Excluded from the xlsx (GATE 3 STOP batches, rows marked `gate3: "STOP"` in the jsonl): 0 rows (-).
* Rows still missing of 4,064: {"count": 0, "ranges": []}.
* Largest `structure_json` cell: 2936 chars (Excel limit 32,767), exercise_id 36878.
* `lk`, `lk_verdict`, `lk_reason` of every row judged in 2H come from the dedicated lk pass (prompt sha16 `5fa910459c078539`, acceptance rule in `lk_sessions_2h.json`).

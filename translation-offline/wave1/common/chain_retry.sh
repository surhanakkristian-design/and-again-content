#!/bin/bash
# tr/hu RETRY chain (decision 55; copy of chain_w1_sub.sh with W1_RETRY=1 and D=wave1/retry_trhu/<lang>).
...
set -u
LANG_=$1
case $LANG_ in tr|hu) ;; *) echo "retry chain: tr|hu only"; exit 1;; esac
export W1_RETRY=1
W1=/Users/kristiansurhanak/Projects/and-again-content/translation-offline/wave1
C=$W1/common; D=$W1/retry_trhu/$LANG_; E=$D/partD; mkdir -p $D
PY="python3 -B $C/pipeline_w1.py $LANG_"
export PYTHONDONTWRITEBYTECODE=1
commit(){ python3 -B $C/gitc.py "Retry $LANG_: $1" $D >/dev/null 2>&1 || true; }
...
step freeze
( cd $W1 && { ls common/*.py common/*.sh spec/*.txt; echo retry_trhu/$LANG_/partA_live/rows.jsonl; echo retry_trhu/$LANG_/partD/set/items.jsonl; echo retry_trhu/$LANG_/partD/set/truth.jsonl; } \
  | sort -u | xargs shasum -a 256 > $E/FROZEN_SHA_D.txt ) || fail "freeze sha"
shasum -a 256 $E/FROZEN_SHA_D.txt | cut -d' ' -f1 > $E/FREEZE_HASH.txt
python3 -B $C/gitc.py "Retry $LANG_: Part D freeze (FROZEN_SHA_D, hash $(cat $E/FREEZE_HASH.txt))" $D $W1/common $W1/spec >/dev/null || fail "freeze commit"
FC=$(git -C $W1 rev-parse HEAD); echo $FC > $E/FREEZE_COMMIT.txt
git -C $W1 ls-tree -r --full-tree --name-only $FC translation-offline/wave1/retry_trhu/$LANG_/partD/FROZEN_SHA_D.txt | grep -q FROZEN_SHA_D || fail "freeze commit missing file"

...
NEW = {r['jid']: r for r in jl(P2J + '/run_S2/results.jsonl')}
N44 = set(json.load(open(P2J + '/partA/NEW_L3_CALLS_S2.json'))['jids'])
st = json.load(open(P2J + '/run_S2/RUN_STATUS.json'))
...
json.dump(res, open(P2J + '/partA/A3_FINAL.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=1)
L = ['## A3 FINAL (stage S2) — %s' % res['label'], '',
     'Run: phase2j/run_S2 via run_2j.py (fixed TRANSLATION-ONLY stack, S2 hook fix): %(requests)s L3 requests, %(seeded_used)s answered by stored 2I replies (0 cost), %(needed)s new, %(calls_made)s calls made (counted %(counted_total)s, uncounted %(uncounted_attempts)s), $%(spend_usd)s.' % res['run_status'], '',
     '| | coverage | FA |', '|---|---|---|',
     '| 2I (before) | %s | %s |' % (fmt(b[0]), fmt(b[1])), '| A2 fix (after) | %s | %s |' % (fmt(a[0]), fmt(a[1])), '',
...
open(P2J + '/partA/A3_FINAL.md', 'w', encoding='utf-8').write('\n'.join(L) + '\n')

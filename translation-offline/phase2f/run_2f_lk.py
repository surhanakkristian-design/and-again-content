...
LEDGER = os.path.join(H, "ledger_2f_lk.json")
LOGP = os.path.join(H, "run_2f_lk.log")
DEFECTS = os.path.join(H, "defects_2f_lk.txt")
...
def session(sid, bid, rows):
    ...
    try:
        ...
        commit([sess_path(sid), LEDGER], "Phase 2F: lk session %s (%d tok)" % (sid, tok_total))
        if out is None:
...
def merge(rows, raw, tasks):
    ...
    commit([cp_path, fin_path], "Phase 2F: lk corrections + final sk annotations (%d rows)" % len(merged))
...
def gate3(anns):
    ...
    commit([os.path.join(OUT, "GATE3_2e.json"), os.path.join(OUT, "GATE3_2e.md")],
           "Phase 2F: GATE 3 on 250 merged rows (seed %d)" % GATE3_SEED)
    return g
...
def report(rows, corrected, gate, stats):
    ...
    commit([rp, LOGP, LEDGER, DEFECTS], "Phase 2F: lk re-judge report (PAR=2, %d tok)" % STATE["tok"])
...
def main():
    ...
    if not A.merge_only:
        ...
        if flagged:
            ...
            commit([LOWDIR], "Phase 2F: low-mode re-run of %d lk session(s)" % len(flagged))
        save_ledger()

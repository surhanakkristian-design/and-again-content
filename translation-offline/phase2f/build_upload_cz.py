...
def main():
    ...
    rp = os.path.join(OUT, "UPLOAD_README.md")
    base = open(os.path.join(D2OUT, "UPLOAD_README.md"), encoding="utf-8").read()
...
""".format(n=len(rows), k=nkeys, b=biggest, h=32767.0 / max(1, biggest), bid=big_id)
    ...
    subprocess.run(["git", "commit", "-q", "-m", "Phase 2E: Czech upload package (%d rows, sheet cz) + README addendum" % len(rows)], cwd=REPO)
